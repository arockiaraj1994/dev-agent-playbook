...
from quality_rules import RuleContext
...
def _ctx(
    tmp_path: Path, files: list[RuleDoc] | None = None, on_disk_index: str | None = None
) -> RuleContext:
    return RuleContext(
...
def test_universal_fails_when_no_h1(tmp_path: Path) -> None:
    doc = _doc(
        "patterns/foo.md", "no heading at all\n", metadata={"title": "Foo", "description": "x"}
    )
    fs = score_doc(doc, _ctx(tmp_path))
...
def _good_agents_body() -> str:
    return (
        "# AGENTS.md - Foo (java)\n\n"
        + (
            "Identity paragraph that explains the agent's role and stack and how this codebase works. "
            * 30
        )
...
def test_dod_good(tmp_path: Path) -> None:
    ...
    doc = _doc(
        "core/definition-of-done.md", body, metadata={"title": "DoD", "description": "checks"}
    )
    fs = score_doc(doc, _ctx(tmp_path))
    assert fs.indicator == "green", fs.failed
...
def test_score_project_real_apache_camel_is_red_only_for_known_failures() -> None:
    ...
    # Counts must add up.
    assert (
        status.counts["red"] + status.counts["amber"] + status.counts["green"] == status.total_files
    )
...
def test_score_project_indicator_amber_when_only_soft_fails(tmp_path: Path) -> None:
    ...
    assert status.indicator in ("green", "amber"), [
        (fs.relative_path, fs.indicator, [r.rule_id for r in fs.failed]) for fs in status.files
    ]
    assert status.missing_required == []
